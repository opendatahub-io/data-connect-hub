...
impl KubeAuthClient {
    ...
    pub async fn authorize(&self, auth_info: &AuthInfo, namespace: &str, verb: &str) -> Result<(), AuthError> {
        let key = format!("{}:{:?}:{}:{}", auth_info.username, auth_info.groups, namespace, verb);
        let client = self.client.clone();
        ...
        if allowed {
            ...
            Err(AuthError::Forbidden(format!(
                "access denied for {RESOURCE} in namespace {namespace}"
            )))
        }
    }
}
