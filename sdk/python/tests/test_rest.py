...
class TestConnectionTypes:
    ...
    async def test_create(self) -> None:
        ...
        result = await client.create_connection_type(req)
        assert result.id == "ct-1"
    ...
    async def test_update(self) -> None:
        ...
        await client.update_connection_type("ct-1", req)
    ...
    async def test_delete(self) -> None:
        ...
        client = _make_client(transport)
        await client.delete_connection_type("ct-1")
...
class TestJsonParseSafety:
    async def test_non_json_response_raises_dch_error(self) -> None:
...
class TestListUnwrapping:
    async def test_handles_wrapped_items_response(self) -> None:
        transport = _make_transport(
    ...
    async def test_handles_bare_array_response(self) -> None:
        transport = _make_transport(
            body=[SAMPLE_CONNECTION_JSON],
            assert_method="GET",
        )
...
class TestRetry:
    ...
    async def test_retries_on_429_respects_retry_after(self) -> None:
        call_count = 0
    ...
    async def test_no_retry_on_non_idempotent_by_default(self) -> None:
        call_count = 0
        ...
        with pytest.raises(DCHServerError):
            await client.create_connection(req)
        assert call_count == 1
